...
module reftet
contains
  pure integer function find_kpoint2(Nk, k, kvec) result(outp)
    implicit none

    integer, intent(in) :: Nk, k(Nk,3), kvec(3)
  ...
  subroutine symmetrize_kmesh(nskold,nk,k,skold,nop,symop,Ndim,nsk,map)
    ...
    integer, intent(in)    :: Nskold, Nk, Nop
    !on input  k: unsymmetrized klist
    !on output k: symmetrized klist with rest zeros
    integer, intent(inout) :: k(nk,3), map(nk,2)
    integer, intent(in)    :: symop(3,3,Nop), Ndim(3), skold(nskold,3)
    integer, intent(out)   :: Nsk

    integer :: jk,js,jd1,jd2
    integer :: tmpk(nk*nop,5),tmpk2(nk*nop,5)
    integer :: tmpk3(nk*nop,5),lock,mapidx(nk*nop),mapcount
    integer :: rotvec(3)
    integer :: counter,jk2
  ...
  subroutine symmetrize_tetra(maxk,map,nt,tetra,wtetra,countsymtetra)
    ...
    integer,   intent(in)    :: maxk, Nt, map(maxk)
    integer,   intent(inout) :: tetra(Nt,10)
    real(DPk), intent(inout) :: wtetra(nt)
    integer,   intent(out)   :: countsymtetra
    ...
    do jd=2,10
       ...
       do while (ichang == 1)
          ...
          do jt=2,nt
             compvec = 0
             do jc=1,jd-1
                if (tmpt(jt,jc) == tmpt(jt-1,jc)) then
                   compvec = compvec + 1
                endif
             enddo
             if ((compvec == jd-1).and.(tmpt(jt,jd) < tmpt(jt-1,jd))) then
                do jc=jd,10
                   it(jc)=tmpt(jt-1,jc)
    ...
    wtetra = 0d0
    do while (jt <= nt)
       if ((tmpt(jt,1) == loctetra(1)).and.(tmpt(jt,2) == loctetra(2)).and.&
            (tmpt(jt,3) == loctetra(3)).and.(tmpt(jt,4) == loctetra(4)).and.&
            (tmpt(jt,4) == loctetra(4)).and.(tmpt(jt,5) == loctetra(5)).and.&
            (tmpt(jt,6) == loctetra(6)).and.(tmpt(jt,7) == loctetra(7)).and.&
            (tmpt(jt,8) == loctetra(8)).and.(tmpt(jt,9) == loctetra(9)).and.&
            (tmpt(jt,10) == loctetra(10))) then
  ...
  subroutine nullestimator(nt,nkfull,nk,tetra,tclass,wtetra,map,NE,nsig,kcontribw,theta,trefine,inter,dE)
    ...
    integer   :: jt,jd
    integer   :: jsig,iE,reftet(8,4)
    integer   :: jref
    real(DPk) :: estimator(nt),maxest,sumest
    real(DPk) :: est1,est2,est3,contrw
    ...
    do iE=0,NE
       ...
       do jsig=1,nsig
          do jt=1,nt
             ...
             if (tclass(jt) == 1) then !central octahedron
  ...
  subroutine refine_tetra(nt,tetra,tclass,wtetra,nkfull,maxk,newk,nvoe,VOE,VOEidx,trefine,maxt, &
    ...
    integer,   intent(in)    :: Nt, Nkfull, maxk, maxt, tetra(Nt,10), tclass(Nt)
    integer,   intent(inout) :: Nvoe, newk(maxk,3), VOE(maxk,300,2)
    integer,   intent(out)   :: VOEidx(Nvoe+48*Nt,3), newtetra(maxt,10), newtclass(maxt), Nnewk, tcount
    real(DPk), intent(in)    :: wtetra(Nt)
    logical,   intent(inout) :: trefine(maxt)

    integer jt,npidx(12,2)
    integer jd,jk,kidx(6),locv,nev,jv
    integer eidx1,eidx2,eidx11,eidx22
    real(DPk) :: newwtetra(maxt)
    logical   :: tchange
             ...
             if ( tetra(jt,1)==tetra(jt,2) .or. &
                ...
                do jd=0,7
                   ...
                   do jk=1,6
                      ...
                      nev = VOE(eidx1,1,1)
                      do jv=1,nev
                         if (VOE(eidx1,jv+1,1) == eidx2) then
          ...
          if (.not.trefine(jt)) then
             ...
             do jk=1,12
                ...
                if (locv.ne.0) then
                   ...
                   nev = VOE(eidx11,1,1)
                   do jv=1,nev
                      if (VOE(eidx11,jv+1,1) == eidx22) then
                         trefine(jt) = .true.
                         tchange = .true.
                         exit
                      endif
  ...
  subroutine get_initialmesh(init_steps,nt,tetra,wtetra,tclass,nkfull,kfull,nvoe,VOE,VOEidx)
    ...
    integer,   intent(in)  :: init_steps
    integer,   intent(out) :: Nt, tetra(48*8**(init_steps-1),10), tclass(48*8**(init_steps-1)), Nkfull, kfull((3*2**init_steps)**3,3), Nvoe, VOE((3*2**init_steps)**3,300,2), VOEidx(6*48*8**(init_steps),3)
    real(DPk), intent(out) :: wtetra(48*8**(init_steps-1))

    integer   :: counter,j1,j2,j3,kmax,maxt
    integer   :: Ti(4),krot(4,3)
    integer   :: newpoints(6,3),kidx(6)
    integer   :: newtetra(48*8**(init_steps-1),10),newtclass(48*8**(init_steps-1))
    real(DPk) :: S(3,3,48),newwtetra(48*8**(init_steps-1))
    integer   :: eidx1,eidx2,jd,jk,jr,js,jt,nnewk,tcount,npidx(6,2)
    integer   :: jsign, jperm
    ...
    do js=1,48
       ...
       do jt=1,4
          do jk=1,nkfull
             if ((krot(jt,1) == kfull(jk,1)).and.(krot(jt,2) == kfull(jk,2))&
                  .and.(krot(jt,3) == kfull(jk,3))) then
                tetra(js,jt) = jk
             endif
          enddo
       enddo
  ...
  subroutine compute_shapeparameters(nk,nt,k,tetra,ndim)
    ...
    integer, intent(in) :: Nk, Nt, k(Nk,3), tetra(Nt,10), Ndim(3)
    ...
    counter = 0
    do jt=1,nt
       if (.not.any(quals(1:counter) == qual(jt))) then
          counter = counter + 1
...
!!!    Sort with heapsort (from Numerical Recipes, p.233)
  subroutine indexx(arrin,indx,n)
    ...
    integer, intent(out) :: indx (size(arrin))
    integer :: n, j, l, i, indxt, ir, q
...
program refine_tmesh
  use reftet,    only: refine_tetra, compute_shapeparameters, nullestimator,&
       &               get_initialmesh, symmetrize_kmesh, symmetrize_tetra
  ...
  character(*), parameter :: rev_str = "$version: v0.1.0-81-gc3a6371$"
...
!!!------------- Write updated files            -----------------------------
  ...
  do jk=1,nkfull+nnewk
     ...
     if (jk == 1) then
        write(unit_fklist,fmt_head) jk, nint(rvec),ndim(1), &
             rdum,-7.,1.5,nkfull+nnewk,kdim
     else
        write(unit_fklist,fmt_line) jk, nint(rvec),ndim(1),rdum
     endif
  enddo
  write(unit_fklist,fmt_end)
  close(unit_fklist)
  ...
  do jk=1,nnewsk
     ...
     if (jk == 1) then
        write(unit_klist,fmt_head) jk, nint(rvec),ndim(1), &
             rdum,-7.,1.5,nnewsk,kdim
     else
        write(unit_klist,fmt_line) jk, nint(rvec),ndim(1),rdum
     endif
  enddo
  write(unit_klist,fmt_end)
  close(unit_klist)
  ...
  do jk=nk+1,nnewsk
     ...
     if (jk == nk+1) then
        write(unit_kadd,fmt_head) jk,nint(rvec),2*kdiv, &
             rdum,-7.,1.5,nnewsk,kdim
     else
        write(unit_kadd,fmt_line) jk, nint(rvec),2*kdiv,rdum
     endif
  enddo
  write(unit_kadd,fmt_end)
  close(unit_kadd)
